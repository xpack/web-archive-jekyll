---
title:  xPack xpm v0.11.0 released
sidebar: xpm

summary: "Version **0.11.0** is a new release; it improves the install logic and fixes some bugs."

version: 0.11.0

date: 2021-12-26 11:12:00 +0200

# --e-n-d-f--

# redirect_to: https://xpack.github.io/xpm/blog/2021/12/26/xpm-v0-11-0-released/
...
- [#124] the logic used to list packages failed to identify scoped
  npm packages, which required going an extra level down in the folder
  hierarchy; fixed;
- [#119] when installing npm dependencies, only the top package must
  be installed, and a recursive search should not be performed; fixed;
- [#120] in certain conditions, `xpm list` failed with exceptions; fixed;

### Enhancements

- [#123] for consistency reasons, accept `-32` as an alias for `--force-32bit`
  in `xpm install`;
- [#122] the logic used by `xpm install` to install all dependencies included
  the dependencies for all build configurations, which was inefficient for
  large projects; the behaviour was reverted to install only the package
  dependencies; to install all dependencies, use `--all-configs`;

### Other changes

- [#121] for a better compatibility when satisfying dependencies for the
   npm packages, the logic used by
  `xpm install` now invokes `npm install` first and later installs the
  xPacks;

### Known problems

- [#125] this release introduced a regression affecting some of the
  `xpm install --config` use cases; fixed in v0.11.1;
